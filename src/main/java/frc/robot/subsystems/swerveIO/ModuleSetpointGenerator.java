...
public class ModuleSetpointGenerator {
  ...
  public ModuleSetpointGenerator(
  ...
  public Double[] computeModuleScrubs(SwerveModuleState[] states) {
    ...
    for (int i = 0; i < states.length; i++) {
      ...
      var cart2 =
          RedHawkUtil.pol2cart(states[i].speedMetersPerSecond, states[i].angle.getRadians());
  ...
  private static SwerveModuleState optimizeModule(
    ...
    double targetAngle =
        RedHawkUtil.placeInScope(desiredState.angle.getRadians(), currentState.angle.getRadians());
